...
public class BDSimulado {
	...
	public String getInfoCliente(String nome){
		...
		for (int i = 0; i < this.clientes.size(); i++) {
			if (this.clientes.get(i).getName().equals(nome)) {
				resultado = "Nome: " + this.clientes.get(i).getName()
						+ "\n" +
						"CPF: " + this.clientes.get(i).getCPF()
						+ "\n" +
					    "Data de Nascimento: " + this.clientes.get(i).getDataNascimento()
						+ "\n" +
	...
	public ArrayList<Integer> ligacoesPorCliente(String nome){
		...
		for (int i = 0; i < this.ligacoes.size(); i++) {
			if (this.ligacoes.get(i).getCliente().getName().equals(nome)) {
	...
	public ArrayList<Integer> ligacoesUFDiferente(){
		...
		for (int i = 0; i < this.ligacoes.size(); i++) {
			if (this.ligacoes.get(i).getCliente().getUF() == this.ligacoes.get(i).getUFDest()) {
				resultado.add(this.ligacoes.get(i).getCodigo());
			}
		}
	...
	public String contaCliente(String nome){
		...
		for (int i = 0; i < this.ligacoes.size(); i++) {
			if (this.ligacoes.get(i).getCliente().getName().equals(nome)) {
				resultado = "Nome: " + this.ligacoes.get(i).getCliente().getName()
						+ "\n" +
						"CPF: " + this.ligacoes.get(i).getCliente().getCPF()
						+ "\n" +
					    "Data de Nascimento: " + this.ligacoes.get(i).getCliente().getDataNascimento()
						+ "\n" +
					    "Telefone: " + this.ligacoes.get(i).getCliente().getTelefone()
					    + "\n" +
					    "Cidade: " + this.ligacoes.get(i).getCliente().getCidade()
					    + "\n" +
					    "UF: " + this.ligacoes.get(i).getCliente().getUF()
						+ "\n" +
					    "Ligações: " + this.ligacoes.get(i).getCodigo() + "\n" + this.ligacoes.get(i).getDataIni()
					    + "\n" + this.ligacoes.get(i).getNumTelDestino() + "\n" + this.ligacoes.get(i).getUFDest();
			}
		}
